...
// Positions 根据GEOPOS指令封装，帮助查询返回结果位置
func Positions(result interface{}, err error) ([]*[2]float64, error) {
	...
	for i := range values {
		...
		if !ok {
			return nil, fmt.Errorf("unexpected element type: %T", value)
		}

		if len(p) != 2 {
			return nil, fmt.Errorf("unexpected number for member, got numbers: %d", len(p))
		}
