...
abstract class SingleChildCloneableWidget implements Widget {
  ...
  /// 
  /// Note for implementers: all other values, including [Key] must be preserved
  SingleChildCloneableWidget cloneWithChild(Widget child);
}

/// A generic implementation of an [InheriedWidget].
/// 
...
class InheritedProvider<T> extends InheritedWidget {
  ...
  /// 
  /// Mutatin `value` should be avoided. Instead rebuild the widget tree
...
/// It is used to imiprove readability and reduce boilderplate code of having to 
/// nest multiple layers of providers.
/// 
/// As such, we'er going from:
/// 
/// ```dart
/// Provider<Foo>.value(
///   value: foo,
...
/// A [Provider] that manages the lifecycle of the value it providers by
...
/// 
/// 
/// Note this exsample purposefully specified the object type, instead of having
/// it infered. 
/// Since we used a mocked class (typically using `mokito`), the we have to 
...
class Provider<T> extends ValueDelegateWidget<T> implements SingleChildCloneableWidget {
  ...
  /// 
  /// The value can be optionally disposed using [dispose] callback. This callback
  /// which will be called when [Provider] is unmounted from the widget tree,
  /// or if [Provider] is result to use [Provider.value] instead.
  Provider({
    Key key,
    ...
    key: key,
    delegate: 
      BuilderStateDelegate<T>(create, dispose: dispose),
  ...
  /// [State.build] to widgets, and [State.didChangeDependencies] for [StatefulWidget].
  static T of<T>(BuildContext context, {bool listen = true}) {
    /// this is required to get generic Type
    // final type = _typeOf<InheritedProvider<T>>();
    final provider = listen ? context.dependOnInheritedWidgetOfExactType<InheritedProvider>() as InheritedProvider<T> : 
                context.getElementForInheritedWidgetOfExactType<InheritedProvider>()?.widget as InheritedProvider<T>;
    if (provider == null) {
  ...
  /// or a [Stream]. In release mode, [debugCheckInvalidValueType] does nothing.
  /// 
  /// This check can be disabled altogether by setting [debugCheckInvalidValueType] to `null` 
  /// like so:
  /// 
  /// ```dart
  /// void main() {
