...
/// 
/// It works with any [InheritedWidget]. Variants like [Selector] and [Selector6]
/// are just syntax sugar to use [Selector0] with [Provider.of]. 
/// 
/// But it will **not** work with values coming from anything but [InheritedWidget].  
/// 
...
class Selector0<T> extends StatefulWidget implements SingleChildCloneableWidget {
  ...
  /// or if [selector] return a value that is different from the previous one using [operator==]. 
  /// 
  /// Must not be `null`.
  final ValueWidgetBuilder<T> builder;

  /// A function that obtains some [InheritedWidget] and map their content into a new
  /// object with only a limited number of properties.
  /// 
  ...
  @override
  _Selector0State<T> createState() => _Selector0State();
...
/// An equivalent to [Consumer] that can filter updates by selecting a limited amount of
/// values and prevent rebuild if they don't change.  
/// 
/// [Selector] will obtain a value using [Provider.of], then pass that value to `selector`. 
/// That `selector` callback is then tasked to return an object that contains only the 
/// informations needed for `builder` to complete.
/// 
/// The object returned by `selector` should be immutable and override [operator==] 
/// such that two objects with the same content are equal, even if they are not
/// [identical]. 
/// 
