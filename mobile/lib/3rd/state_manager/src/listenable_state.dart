...
import 'change_notifier_state.dart' show 
...
class ListenableProvider<T extends Listenable> 
  ...
  /// 
  /// [dispose] can optionally passed to free resource when [ListenableProvider]
  ...
  ListenableProvider({
    ...
    key: key, 
    delegate: _BuilderListenableDelegate(create, dispose: dispose), 
    child: child,
  );
 ...
 extends SingleValueDelegate<T> with _ListenableDelegateMixin<T> {
  ...
  void didUpdateDelegate(_ValueListenableDelegate<T> oldDelegate) {
    ...
    if (oldDelegate.value != value) {
      ...
      if (value != null) {
        startListening(value, rebuild: true);
      }
    }
  }
...
mixin _ListenableDelegateMixin<T extends Listenable> on ValueStateDelegate<T> {
  ...
  void startListening(T listenable, {bool rebuild = false}) {
    ...
    /// the listenable emitting an event. 
    /// This allows [InheritedWidget.updateShouldNotify] to return true only in
    /// the latter scenario
    var buildCount = 0;
    final setState = this.setState;
    final listener = () => setState(() => buildCount++);

    var captureBuildCount = buildCount;
    // purposefully desynchronize buildCount and captureBuildCount
    // after an update to ensure that the first updateShouldNotify returns true
    if (rebuild) {
      captureBuildCount--;
    }
    updateShouldNotify = (_, __) {
      final res = buildCount != captureBuildCount;
      captureBuildCount = buildCount;
      return res;
    };
...
/// A variation of [ListenableProvider] that builds its value from values
...
/// But [ListenableProxyProvider] is exposed in case one wants to use a [Listenable]
/// implementation other than [ChangeNotifier], such as [Animation].  
/// {@endtemplate}
