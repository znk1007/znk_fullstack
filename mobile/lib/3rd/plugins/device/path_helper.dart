...
/// Path to the temporary directory on the device that is not backed up
/// and is suitable for storing caches of downloaded files.
/// 
/// Files in this directory may be cleared at any time. 
/// This does *not* return a new temporary directory.
/// Instead, the caller is reponsible for creating (and cleaning up) files
/// or directories within this directory. This directory is scoped to the calling application.
/// 
/// On iOS, this uses the `NSCachesDirectory` API.
/// 
/// On Android, this uses the `getCacheDir` API on the context.
Future<Directory> getTemporaryDirectory() async {
  final String path = await methodChannel.invokeMethod<String>('getTemporaryDirectory');
  if (path == null) {
    return null;
  }
...
Future<Directory> getApplicationSupportDirectory() async {
  final String path =
      await methodChannel.invokeMethod<String>('getApplicationSupportDirectory');
  if (path == null) {
    return null;
  }
...
Future<Directory> getApplicationDocumentsDirectory() async {
  final String path =
      await methodChannel.invokeMethod<String>('getApplicationDocumentsDirectory');
  if (path == null) {
    return null;
  }
  return Directory(path);
...
Future<List<Directory>> getExternalCacheDirectories() async {
  ...
  final List<String> paths =
      await methodChannel.invokeListMethod<String>('getExternalCacheDirectories');
