...
//errInvalidCompression is returned invalid compression
var errInvalidCompression = errors.New("ws: invalid compression negotiation")
...
type Dialer struct {
	...
	//If NetDial is nil, net.Dial is used.
	NetDial func(network, addr string) (net.Conn, error)

	//NetDialConText specifies the dial function for creating TCP connections.
	//If NetDialContext if nil, net.DialContext is used
	...
	//TLSClientConfig TLSClientConfig specifies the TLS configuration to use with tls.Client.
	...
	//If a buffer size is zero, then a useful default size is used. The I/O
	//buffer sizes do not limit the size of the messages that can be sent or received.
	...
	//EnableCompression specifies it the client should attempt to negotiate
...
//non-nil *http.Response so that callers can handle redirects, authentication, etc.
//
...
//to get the selected subprotocol (Sec-WebSocket-Protocol) and cookies (Set-Cookie).
//
...
func (d *Dialer) DialContext(ctx context.Context, urlStr string, requestHeader http.Header) (*Conn, *http.Response, error) {
	...
	//method canoicalizes the header names.
	...
	for k, vs := range requestHeader {
		...
		case k == "Upgrade" ||
			...
			return nil, nil, errors.New("ws: duplicate header not allowed: " + k)
		case k == "Sec-Websocket-Protocol":
			req.Header["Sec-WebSocket-Protocl"] = vs
		default:
			req.Header[k] = vs
		}
	}
	...
	var netDial func(network, addr string) (net.Conn, error)
