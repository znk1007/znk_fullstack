...
//NextWriter and SetWriteDeadline, it return io.EOF.
//
...
//Pause pauses the payload. It will wait all reader and writer closed
//which created from NextReader or NextWriter.
...
func (p *Payload) setReader(err error) error {
	select {
	case <-p.close:
		return p.load()
	default:
...
func (p *Payload) setWriter(err error) error {
	select {
	case <-p.close:
		return p.load()
	default:
