...
//FrameWriter interface of write data frame
type FrameWriter interface {
	NextWriter(ft ngxio.FrameType) (io.WriteCloser, error)
}
...
//NewEncoder new the data encoder
...
//Encode the data from transport
func (e *Encoder) Encode(h Header, args []interface{}) (err error) {
	var w io.WriteCloser
	w, err = e.w.NextWriter(ngxio.TEXT)
	if err != nil {
		return
	}
	var bufs [][]byte
	...
	for _, b := range bufs {
		w, err = e.w.NextWriter(ngxio.BINARY)
		if err != nil {
			return
		}
...
func (e *Encoder) writePacket(w io.WriteCloser, h Header, args []interface{}) ([][]byte, error) {
	...
	if len(bufs) > 0 && (h.Type == Event || h.Type == Ack) {
		h.Type += 3
	}
