...
/// Builder callback which return Widget
typedef ProviderBuilder<R> = Widget Function(
  BuildContext context, 
  R value, 
  Widget child
);
/// Proxy buidler callback which return generic value
...
abstract class ProxyProviderState<T extends ProxyProviderWidget> extends State<T> {
  /// To not confuse with [didChangeDependencies], [didUpdateDependencies] is guaranteed 
  /// to be followed by a call to [build], and will be called only once after _all_
  /// dependencies have changed.
  /// 
  /// This guarantees that everything is up to date when [didUpdateDependencies] is called,
  /// and that the widget will not be unmounted before updates are applied. It is therefore
  /// safe to make network http calls or mutations inside this life-cycle.
  @protected
  @mustCallSuper
...
abstract class ProxyProviderBase<R> extends ProxyProviderWidget {
  ...
  ProxyProviderBase({
    ...
    this.dispose, 
  }) : initialBuilder = create, super(key: key);
  ...
  @override
  ProxyProviderState<ProxyProviderWidget> createState() => _ProxyProviderState();
...
class NumericProxyProvider<T, T2, T3, T4, T5, T6, R> 
 extends ProxyProviderBase<R> implements SingleChildCloneableWidget {
  ...
  /// 
  /// [builder] will be called once when the widget is mounted, and once whenever any of
  /// the [InheritedWidget] which [ProxyProvider] depends on updates.
  /// 
  /// It is safe to perform side-effects in this method.
  /// {@endtemplate}
  ...
  R didChangeDependencies(BuildContext context, R previous) {
    ...
    if (T2 != Void) {
      arguments.add(Provider.of<T2>(context));
    }
    if (T3 != Void) {
      arguments.add(Provider.of<T3>(context));
    }
    if (T4 != Void) {
      arguments.add(Provider.of<T4>(context));
    }
    if (T5 != Void) {
      arguments.add(Provider.of<T5>(context));
    }
    if (T6 != Void) {
      arguments.add(Provider.of<T6>(context));
    }
...
/// more than once. It will be called once when the widget is mounted, the once
...
/// of arguments. The `2` in [ProxyProvider2] means that `update` builds its value 
/// from **2** other providers. 
/// 
/// All variations of `update` will receive the [BuildContext] as first parameter,
/// and the previously built value as last parameter. 
/// 
/// This previously built value will be `null` by default, unless `create`
/// is specified - in which case, it will be the value returned by `create`.
/// 
/// `update` must not be `null`. 
/// 
/// ##Note:
/// 
/// While [ProxyProvider] has built-in support with [Provider.of], it works with
/// *any* [InheritedWidget]. 
/// 
...
/// )
/// ```
/// And it will also work with other `.of` patterns, include [Scrollable.of],
/// [MediaQueury.of], and many more:
/// 
/// ```dart
/// ProxyProvider<Foo, Baz>(
...
/// ```
/// 
/// This previous example will correctly rebuild `Baz` when the `MediaQuery` update. 
/// 
/// See also:
/// 
...
class ProxyProvider<T, R> 
  ...
  @override
  ProxyProviderBuilder<T, R> get builder => super.builder as ProxyProviderBuilder<T, R>;
}
...
class ProxyProvider2<T, T2, R> 
  ...
  @override
  ProxyProviderBuilder2<T, T2, R> get builder => super.builder as ProxyProviderBuilder2<T, T2, R>;
}
...
class ProxyProvider3<T, T2, T3, R> 
  ...
  @override
  ProxyProviderBuilder3<T, T2, T3, R> get builder => super.builder as ProxyProviderBuilder3<T, T2, T3, R>;
}
